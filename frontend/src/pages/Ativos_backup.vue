    ...
    <div v-if="modalAberto" class="modal-overlay" @click="fecharModal">
      <div class="modal-container" @click.stop>
        ...
        <form @submit.prevent="salvarAtivo" class="modal-form">
          ...
          <div class="form-group">
            <label class="form-label">Imagens do Ativo (máx. 4)</label>
            <ImagensAtivo
              v-if="ativoEdicao"
              :ativo-id="ativoEdicao.id"
              :editavel="true"
            />
            <div v-else class="imagens-upload-container">
              <div class="upload-container"
                ...
                <div class="upload-options">
                  <div class="upload-info">
                    <i class="fas fa-upload" style="font-size: 32px; color: #6b7280;"></i>
                    <h4>Arraste imagens aqui ou</h4>
                    <p class="size-info">Máx. 5MB por imagem • JPEG, PNG, WebP</p>
                  </div>
                  
                  <div class="upload-buttons">
                    <label for="upload-imagem-novo" class="upload-btn primary">
                      <i class="fas fa-plus-circle" style="font-size: 20px;"></i>
                      <span>Selecionar Imagens</span>
                    </label>
                    <input 
                      id="upload-imagem-novo"
                      type="file"
                      accept="image/jpeg,image/png,image/webp"
                      @change="uploadImagensNovo"
                      multiple
                      class="hidden-input"
                    />
                    
                    <label for="upload-pasta-novo" class="upload-btn secondary">
                      <i class="fas fa-folder" style="font-size: 20px;"></i>
                      <span>Importar Pasta</span>
                    </label>
                    <input 
                      id="upload-pasta-novo"
                      type="file"
                      accept="image/jpeg,image/png,image/webp"
                      @change="uploadPastaNovo"
                      multiple
                      webkitdirectory
                      class="hidden-input"
                    />
              ...
              <div v-if="imagensTemporarias.length > 0" class="imagens-preview">
                <div class="preview-grid">
                  <div v-for="(imagem, index) in imagensTemporarias" :key="imagem.id" class="preview-item">
                    <img :src="imagem.preview" :alt="`Imagem ${index + 1}`" class="preview-image" />
                    <button @click="removerImagemTemporaria(imagem.id)" class="remove-btn">
...
const salvarAtivo = async () => {
  try {
    ...
    if (ativoEdicao.value) {
      ...
      ativoSalvo = ativoEdicao.value
      success('Sucesso', 'Ativo atualizado com sucesso')
    } else {
      const response = await api.post('/ativos', formulario.value)
      ativoSalvo = response.data
      success('Sucesso', 'Ativo cadastrado com sucesso')
    }
    
    // Upload das imagens temporárias para novos ativos
    if (!ativoEdicao.value && imagensTemporarias.value.length > 0) {
      await uploadImagensTemporariasPara(ativoSalvo.id)
    }
...
const uploadImagensTemporariasPara = async (ativoId) => {
  for (const img of imagensTemporarias.value) {
    try {
      ...
      console.warn(`Erro ao fazer upload da imagem ${img.nome}:`, err)
    }
  }
}
...
const fecharModal = () => {
  ...
  ativoEdicao.value = null
  imagensTemporarias.value = []
  isDragOver.value = false
}

// Métodos de upload para novo ativo
const uploadImagensNovo = (event) => {
  const files = Array.from(event.target.files)
  adicionarImagensTemporarias(files)
}

const uploadPastaNovo = (event) => {
  const files = Array.from(event.target.files)
  adicionarImagensTemporarias(files)
}
...
const adicionarImagensTemporarias = (files) => {
  const maxFiles = 4
  const maxSize = 5 * 1024 * 1024 // 5MB
  const allowedTypes = ['image/jpeg', 'image/png', 'image/webp']
  
  for (let file of files) {
    if (imagensTemporarias.value.length >= maxFiles) {
      warning('Limite atingido', `Máximo ${maxFiles} imagens por ativo`)
      break
    }
    
    if (!allowedTypes.includes(file.type)) {
      warning('Formato inválido', `Arquivo ${file.name} não é uma imagem válida`)
      continue
    }
    
    if (file.size > maxSize) {
      warning('Arquivo muito grande', `${file.name} excede 5MB`)
      continue
    }
    
    const reader = new FileReader()
    reader.onload = (e) => {
      imagensTemporarias.value.push({
...
const removerImagemTemporaria = (id) => {
  imagensTemporarias.value = imagensTemporarias.value.filter(img => img.id !== id)
}
